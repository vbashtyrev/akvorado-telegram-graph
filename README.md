...
**В консоли выбран L3ᵇ⁄ₛ, а бот присылает другие цифры?** Задайте в `config.yaml` (секция `clickhouse`) `bytes_expression: "greatest(0, Bytes - 18 * Packets) * coalesce(SamplingRate, 1)"` — тогда бот считает L3 (байты без заголовков кадра). Подписи на графике уже L3.

**Цифры расходятся с консолью Akvorado?** Консоль для разных периодов использует разные таблицы. В `config.yaml` (секция `clickhouse`): для 6 ч — `table_6h: "default.flows_1m0s"`, `interval_sec_6h: 60`. Для 24/7 д: если в UI min — десятки Мбит/с, а бот присылает сотни, в консоли почасовая агрегация — задайте `table_24h: "default.flows_1h0m0s"`, `interval_sec_24h: 3600`; иначе попробуйте `flows_5m0s` и `300`. Перезапустите бота после правки.
